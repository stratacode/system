...
public abstract class BodyTypeDeclaration extends Statement implements ITypeDeclaration {
   ...
   public Boolean autoComponent = null;
   ...
   */
   ...
   public Object definesMemberInternal(String name, EnumSet<MemberType> mtype, Object refType, TypeContext ctx, boolean skipIfaces, boolean isTransformed) {
      ...
      if (extendsType != null) {
         ...
         if (extendsTypeName != null && getDeclarationType() == DeclarationType.OBJECT &&
                 ...
                 // current model.
                 (objDecl = getJavaModel().resolveName(extendsTypeName, false, true)) != null)
   ...
   public Object findMethod(String name, List<? extends Object> params, Object fromChild, Object refType) {
      ...
      if ((v = definesMethod(name, params, null, refType, isTransformedType())) != null)
         return v;
   ...
   public Object definesMethod(String name, List<?> types, ITypeParamContext ctx, Object refType, boolean isTransformed) {
      ...
      if (isTransformed && transformedType != null) {
         v = transformedType.definesMethod(name, types, ctx, refType, isTransformed);
         if (v != null)
            return v;
      }

      v = declaresMethod(name, types, ctx, refType);
      if (v != null)
         return v;

      if ((v = extendsDefinesMethod(name, types, ctx, refType, isTransformed)) != null)
         return v;

      return super.definesMethod(name, types, ctx, refType, isTransformed);
   }

   public static Object findMethodInBody(List<Statement> body, String name, List<? extends Object> types, ITypeParamContext ctx, Object refType) {
      Object v = null;
      ...
      if (body != null) {
         ...
         for (Statement s:body) {
            ...
            Object newMeth;
            if ((newMeth = s.definesMethod(name, types, ctx, refType, false)) != null) {
               if (v == null)
                  v = newMeth;
               else {
                  if (typesArray == null)
   ...
   public Object declaresMethod(String name, List<? extends Object> types, ITypeParamContext ctx, Object refType) {
      ...
      v = findMethodInBody(body, name, types, ctx, refType);
      if (v != null)
         return v;
      ...
      if (isTransformedType() && isAutoComponent() && !isTransformed()) {
         ...
         v = ModelUtil.definesMethod(ComponentImpl.class, name, types, ctx, refType, true);
         if (v != null)
            return v;
      }
   ...
   AbstractMethodDefinition declaresMethodDef(String name, List<? extends Object> types) {
      Object res = declaresMethod(name, types, null, null);
   ...
   public Object extendsDefinesMethod(String name, List<?> parameters, ITypeParamContext ctx, Object refType, boolean isTransformed) {
      Object td = getDerivedTypeDeclaration();

      if (td != null)
         return ModelUtil.definesMethod(td, name, parameters, ctx, refType, isTransformed);
   ...
   ArrayList<IDefinitionProcessor> getInheritedDefinitionProcessors() {
      ...
      HashMap<String,IAnnotationProcessor> procs = sys.annotationProcessors;
      for (Map.Entry<String,IAnnotationProcessor> procEnt:procs.entrySet()) {
         String annotTypeName = procEnt.getKey();
         IAnnotationProcessor proc = procEnt.getValue();
         if (proc.getInherited()) {
            Object annot = getInheritedAnnotation(annotTypeName);
            if (annot != null) {
               if (res == null)
   ...
   public Object getInnerType(String name, TypeContext ctx) {
      return getInnerType(name, ctx, true, false);
   }

   public Object getInnerType(String name, TypeContext ctx, boolean checkBaseType, boolean redirected) {
      int ix;
      ...
      do {
         ...
         if (type instanceof BodyTypeDeclaration)
            nextType = ((BodyTypeDeclaration) type).getSimpleInnerType(nextTypeName, ctx, checkBaseType, redirected);
         else
      ...
      */
      ...
      if (checkBaseType) {
         ...
         if (redirected && this instanceof ModifyDeclaration && ((ModifyDeclaration) this).modifyInherited)
            redirected = false;
         return getSimpleInnerTypeFromExtends(extendsType, name, ctx, redirected);
      }

      return null;
   }

   protected Object getSimpleInnerTypeFromExtends(Object extendsType, String name, TypeContext ctx, boolean redirected) {
      Object res;
      if (extendsType != null) {
         if (extendsType instanceof TypeDeclaration) {
            ...
            if ((res = ext.getInnerType(name, ctx, true, true)) != null)
               if (ctx != null)
                  ctx.add(ext, this);
            return res;
         }
   ...
   public Object findMember(String name, EnumSet<MemberType> mtype, Object fromChild, Object refType, TypeContext ctx, boolean skipIfaces) {
      ...
      // else - variable only just return
      return v;
   }
   ...
   public Definition modifyDefinition(BodyTypeDeclaration base, boolean doMerge, boolean inTransformed) {
      ...
      if (otherTypeObj instanceof BodyTypeDeclaration) {
         ...
         if (!(annot.elementValue instanceof StringLiteral)) {
            ...
            Object indexType = base.getInnerType(otherTypeName, null, false, false);
                              ...
                              if (getSelector == null) {
                                 ...
                                 for (int j = i; j < size; j++) {
                                    ...
                                    if (nextStatement instanceof MethodDefinition) {
                                       ...
                                       if (theMethod.name.equals(getName) && theMethod.getParameterTypes().length == 0) {
   ...
   private Object definesComponentMethod(String name) {
      Object res = definesMethod(name, null, null, this, false);
      if (res == null)
         res = definesMethod("_" + name, null, null, this, false); // Check the Alt version _preInit, _init, etc.
      return res;
   }
      ...
      if (isComponent || mgr != null) {
         ...
         try {
            ...
            if (isComponent && doInit) {
               ...
               Object initMethod = definesMethod("init", null, null, this, false);
               if (initMethod == null)
                  initMethod = ModelUtil.definesMethod(rtType, "init", null, null, this, false);
               if (initMethod != null) {
                  initMethod = ModelUtil.getRuntimeMethod(initMethod);
                  try {
   ...
   */
   ...
   public Statement updateBodyStatement(Statement def, ExecutionContext ctx, boolean updateInstances, UpdateInstanceInfo info) {
      ...
      if (def instanceof AbstractMethodDefinition) {
         ...
         AbstractMethodDefinition overridden = (AbstractMethodDefinition) declaresMethod(newDef.name, newDef.getParameterList(), null, null);
         if (overridden != null) {
   ...
   public Statement removeBodyStatement(Statement def, ExecutionContext ctx, boolean updateInstances, UpdateInstanceInfo info) {
      ...
      if (def instanceof AbstractMethodDefinition) {
         ...
         // Look for a method in this type specifically.  Anything in a modified type or extended type should not
         // get replaced since we are overriding that method in that case.
         AbstractMethodDefinition overridden = (AbstractMethodDefinition) declaresMethod(toRem.name, toRem.getParameterList(), null, null);
         if (overridden != null) {
      ...
      if (body != null && (updateMode == TypeUpdateMode.Replace || updateMode == TypeUpdateMode.Remove)) {
         for (int i = 0; i < body.size(); i++) {
            ...
            if (oldBodyDef instanceof FieldDefinition) {
               ...
               Object methObj = newType.definesMethod(oldMeth.name, oldMeth.getParameterList(), null, null, false);
      ...
      if (newType.body != null) {
         for (int i = 0; i < newType.body.size(); i++) {
            ...
            if (newBodyDef instanceof FieldDefinition) {
               ...
               AbstractMethodDefinition oldMeth = (AbstractMethodDefinition) declaresMethod(newMeth.name, newMeth.getParameterList(), null, null);
               if (oldMeth != null) {
                  if (oldMeth == newMeth)
   ...
   public Template findTemplatePath(String templatePath, String templateName, Class paramClass) {
      if (templatePath == null || templatePath.equals(""))
         return null;
      Template template = getJavaModel().getLayeredSystem().getTemplate(templatePath, null, paramClass, null);
      if (template == null) {
   ...
   public ArrayList<SyncProperties> syncProperties = null;

   protected boolean syncPropertiesInited = false;
